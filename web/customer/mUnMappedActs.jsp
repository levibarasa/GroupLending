    ...
    <div class="div">
        <table width="95%" align="center"  style="border:#019ADD solid 2px;padding:10px;" border="1">
            ...
            <tr style="height:30px; padding:4px;">
                <td><div align="center"><%=(int) one.get(0)%></div></td>
                <td><div align="center"><%=(String) one.get(1)%></div></td>
                <td><div align="center"><%=(String) one.get(2)%></div></td>
                <td>
                    <label>
                        <input name="<%= (int) one.get(0) + "subgroup"%>" type="text" readonly="true"  id="<%= (int) one.get(0) + "subgroup"%>" onkeyup="caps(this)" class="textboxes" size="10"/>
                        <input name="<%= (int) one.get(0) + "subgroupName"%>" type="hidden" readonly="true"  id="<%= (int) one.get(0) + "subgroupName"%>" onkeyup="caps(this)" class="textboxes"/>
                    </label>
                </td> <td><a href="" onclick="return getSubGroup('<%=(int) one.get(0)%>')"><img src="images/search.png"></a></td>
                <td>
                    <label>
                        <input type="text" name="<%= (int) one.get(0) + "custType"%>" onkeyup="this.value = this.value.toUpperCase();" value="1" id="<%= (int) one.get(0) + "custType"%>" readonly="true" />
                    </label>
                </td>
                <td>
                    <input value="Add" type="button" class="redButton" onclick="javascript:deleteDriver(<%= (int) one.get(0)%>)" size="6"/>
                </td>
            </tr>
            <% }%>
        </table>
